...
(function($){
		...
		----------------------------*/
	 	_init : function(){
    		...
    		if (api.options.autoplay){
				...
				if (api.options.progress_bar) $(vars.progress_bar).stop().animate({top : -$(window).height()}, 0 );	//  Place progress bar
			}
			...
			//Make Some magic)
			$(vars.thumb_list).find('img').greyScale({fadeTime: 500, reverse: false});
			...
			if ($('.right_bar').hasClass('act')) {
				...
				setTimeout("$(vars.slide_caption).stop().animate({'right' : '80px', 'opacity' : '1'}, 800)",500);
	...
	$('#thumbs-scroller').height($(window).height() - 140)
		...
		$('#thumbs-scroller').stop().animate({'left' : '38px'}, 800, 'easeOutBack');
		setTimeout("$('#thumbs-scroller').stop().animate({'left' : '-100px'}, 800, 'easeInBack')",3000);
			...
			$(window).resize(function(){
				...
				if (api.options.progress_bar && !vars.in_animation){
					...
					$(vars.progress_bar).stop().animate({top : -$(window).height()}, 0 );
		...
		----------------------------*/
	 	playToggle : function(state){
	 		...
	 		if (state =='play'){
				...
				if ($(vars.play_button).attr('href')) $(vars.play_button).removeClass("pause");	// If pause play button is link, addclass
        		if (api.options.progress_bar && vars.is_paused)$(vars.progress_bar).stop().animate({top : -$(window).height()}, 0 );
	 		}
		...
		progressBar : function(){
    		$(vars.progress_bar).stop().animate({top : -$(window).height()}, 0 ).animate({ top:0 }, api.options.slide_interval);
    	}
